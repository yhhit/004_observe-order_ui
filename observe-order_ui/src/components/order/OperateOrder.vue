<template>
  <el-form ref="form" :model="form" :rules="rules" label-width="80px">
      ...
      <el-checkbox-group v-model="form.cycle" :disabled="type === 'read'">
        <div>
          <el-button
            ...
            round
            @click="
              form.cycle = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
            "
            v-if="type !== 'read'"
            >all week</el-button
          >
          <el-button
            ...
            size="mini"
            round
            @click="form.cycle = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri']"
            v-if="type !== 'read'"
            >weekday</el-button
          >
          <el-button
    ...
    <el-form-item style="margin: auto; width: fit-content">
      <el-button type="primary" @click="onSubmit" v-if="type !== 'read'"
        >Issue</el-button
      >
...
export default {
      ...
      rules: {
        title: [
          { required: true, message: "Please input title", trigger: "blur" },
          {
            min: 3,
            max: 30,
            message: "The length of title must between 3 and 30",
            trigger: "blur",
          },
        ],
        toWhom: [
  ...
  async mounted() {
    try {
      let res = await axios.get(toWhomOptionsUrl);
      if (res.data.code === 0) {
